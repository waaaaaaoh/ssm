					...
					<tbody>
						...
						<c:forEach items="${ls.list}" var="goods">
							<tr >
								...
								<td >${goods.price }</td>
								<td >${goods.num }</td>
								<td ><img alt="" src="static/images/pro5.png"></td>
								...
								<nav aria-label="...">
									<ul class="pagination">
									    <c:choose>
									      ...
									      <c:otherwise>
									      <li class="page-item"><a class="page-link" href="goodsnameList?pageNum=${ls.pageNum-1}}&name=${str}" tabindex="-1" aria-disabled="true">Previous</a></li>
									      </c:otherwise>
									    </c:choose>
										...
										<c:forEach begin="1" end="${ls.pages }" var="i">
										  <c:choose>
										    ...
										    <li class="page-item active" aria-current="page"><a class="page-link" href="goodsnameList?pageNum=${i}&name=${str}">${i} <span class="sr-only">(current)</span></a></li>
										    </c:when>
										    <c:otherwise>
										    <li class="page-item"><a class="page-link" href="goodsnameList?pageNum=${i}&name=${str}">${i }</a></li>
										    </c:otherwise>
										  </c:choose>
										</c:forEach>
										...
										<c:choose>
										  ...
										  <c:otherwise>
										  <li class="page-item "><a class="page-link" href="goodsnameList?pageNum=${ls.pageNum+1}&name=${str}">Next</a></li>
										  </c:otherwise>
										</c:choose>
